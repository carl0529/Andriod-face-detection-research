...
public final class Facenet {
  ...
  private Facenet(@NonNull Context context, @NonNull Model.Options options) throws IOException {
    model = Model.createModel(context, "facenet.tflite", options);
    MetadataExtractor extractor = new MetadataExtractor(model.getData());
  }

  @NonNull
  public static Facenet newInstance(@NonNull Context context) throws IOException {
    return new Facenet(context, (new Model.Options.Builder()).build());
  }

  @NonNull
  public static Facenet newInstance(@NonNull Context context, @NonNull Model.Options options) throws
      IOException {
    return new Facenet(context, options);
  }
